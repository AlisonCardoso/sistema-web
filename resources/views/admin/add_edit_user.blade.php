...
<div class="mt-4 mb-lg-4">
    ...
    @endif
            ...
            <form class="form1" method="post" action="@if (isset($edit->id)) {{ route('users.update', ['id' => $edit->id]) }}@else{{ route('users.store') }} @endif" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group mt-4 col-md-12 mb-3">
                    <label for="'email"> E-mail</label>
                    <input class="form-control" type="text" name="email"  value="@if (isset($edit->id)) {{ $edit->email }}@else {{ old('email') }} @endif">
                    @error('email')
                    <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
